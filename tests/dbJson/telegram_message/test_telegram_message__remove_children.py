...
@pytest.mark.asyncio
async def test_file_message__remove_children(TelegramManagerClient_init):
    """
    Test the remove_children method of TelegramMessage with valid input
    """
    try:
        ...
        # Remove the child from the parent and assert that it is removed successfully
        await root_message.remove_children(client, target_chat_instance, TelegramMessage.calculate_message_link(target_chat_instance, folder_message))
...
@pytest.mark.asyncio
async def test_file_message__remove_children__invalid(TelegramManagerClient_init):
    """
    Test the remove_children method of TelegramMessage with invalid link input
    """
    try:
        # Init variables for TelegramManagerClient
        client, target_chat_instance = TelegramManagerClient_init
        ...
        # Attempt to remove an invalid child and assert that an exception is raised
