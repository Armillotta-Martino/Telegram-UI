...
@pytest.mark.asyncio
async def test_telegram_message__add_children(TelegramManagerClient_init):
    """
    Test the add_children method of TelegramMessage with valid input
    """
    try:
        # Init variables for TelegramManagerClient and get the target chat instance
...
@pytest.mark.asyncio
async def test_telegram_message__add_children__invalid(TelegramManagerClient_init):
    """
    Test the add_children method of TelegramMessage with invalid link input
    """
    try:
        # Init variables for TelegramManagerClient and get the target chat instance
        client, target_chat_instance = TelegramManagerClient_init
        ...
        # Attempt to add an invalid child and assert that an exception is raised
        with pytest.raises(ValueError, match="Invalid message link format"):
